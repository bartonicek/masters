...
export class MarkerHandler2 extends Handler {
    ...
    isOfMembership = (index, membership) => {
        ...
        if (currVal > 1)
            return 2 * Math.floor(currVal / 2) <= membership;
        if (pastVal > 1)
            return 2 * Math.floor(pastVal / 2) <= membership;
    };
    replaceCurrent = (at, membership) => {
...
export class MembershipArray2 extends Uint8Array {
    ...
    replace = (at, membership) => {
        let i = at.length;
        if (membership === 1) {
            // If membership is 1 (transient), increments idempotently by 1
            while (i--)
                this[at[i]] = 2 * Math.floor(this[at[i]] / 2) + 1;
            return;
        }
