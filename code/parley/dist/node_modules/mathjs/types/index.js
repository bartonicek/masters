import { create, factory, all, fractionDependencies, addDependencies, divideDependencies, formatDependencies, } from 'mathjs';
...
*/
{
    ...
    const b = math.add(math.matrix([2]), math.matrix([3]));
    const c = math.subtract(math.matrix([4]), math.matrix([5]));
}
/*
Bignumbers examples
*/
...
*/
{
    ...
    {
        const p = { r: math.sqrt(2), phi: math.pi / 4 };
        const c = math.complex(p);
    }
    // get polar coordinates of a complex number
    {
        const p = math.complex(3, 4).toPolar();
    }
}
/*
...
*/
{
    ...
    {
        const scope = { hello: (name) => `hello, ${name}!` };
    ...
    // get and set variables and functions
    {
        ...
        const g = f(3, 3);
        parser.set('h', 500);
        parser.set('hello', (name) => `hello, ${name}!`);
    }
...
*/
{
    ...
    // create and manipulate matrices. Arrays and Matrices can be used mixed.
    {
        ...
        const f = math.matrix([1, 0]);
        const d = f.subset(math.index(1));
    }
    // get a sub matrix
    {
        const a = math.diag(math.range(1, 4));
        a.subset(math.index([1, 2], [1, 2]));
        const b = math.range(1, 6);
    ...
    // map matrix
    {
        assert.deepStrictEqual(math.map([1, 2, 3], function (value) {
            return value * value;
        }), [1, 4, 9]);
    }
    // filter matrix
    {
        assert.deepStrictEqual(math.filter([6, -2, -1, 4, 3], function (x) {
            return x > 0;
        }), [6, 4, 3]);
...
*/
{
    ...
    const filtered = node.filter((node) => node.type === 'SymbolNode' && node.name === 'x');
    const arr = filtered.map((node) => node.toString());
    // Traverse an expression tree
    const node1 = math.parse('3 * x + 2');
    node1.traverse((node, path, parent) => {
        switch (node.type) {
            case 'OperatorNode':
